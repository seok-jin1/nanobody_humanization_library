...
| 파일 | 설명 |
|------|------|
| `fetch_vh3_alleles.py` | OGRDB REST API에서 VH3 allele 서열 다운로드 |
| `translate_to_aa.py` | DNA → 아미노산 번역 후 CSV 갱신 |
| `imgt_numbering.py` | unique 서열 추출 + ANARCI IMGT numbering |
| `aa_proportion.py` | IMGT position별 아미노산 비율 계산 |

### 데이터 파일

| 파일 | 설명 |
|------|------|
| `VH3_alleles_gapped.fasta` | IMGT gap 포함 DNA FASTA (93 alleles) |
| `VH3_alleles_ungapped.fasta` | gap 제거 DNA FASTA (93 alleles) |
| `VH3_alleles_summary.csv` | allele별 DNA + 아미노산 서열 (93 alleles) |
| `VH3_unique.fasta` | unique 아미노산 서열 FASTA (67 seqs) |
| `VH3_unique_imgt.csv` | unique 서열별 IMGT position 컬럼 CSV (67 seqs × 118 positions) |
| `VH3_imgt_aa_proportion.csv` | position별 아미노산 비율 wide format |
| `VH3_imgt_aa_proportion_long.csv` | position별 아미노산 비율 long format |

---

## 1. 서열 다운로드 (`fetch_vh3_alleles.py`)
...
## 2. 아미노산 번역 (`translate_to_aa.py`)

`VH3_alleles_summary.csv`의 `sequence` 컬럼(ungapped DNA)을 **표준 유전암호(standard genetic code)** 로 번역하여 `aa_sequence` 컬럼을 추가한다.
...
## 3. IMGT Numbering (`imgt_numbering.py`)
...
## 4. 아미노산 비율 계산 (`aa_proportion.py`)

`VH3_unique_imgt.csv`를 읽어 **IMGT position별 아미노산 출현 비율**을 계산한다.
...
```bash
# 1. 서열 새로 다운로드
python3 fetch_vh3_alleles.py

# 2. DNA → 아미노산 번역 (CSV 갱신)
python3 translate_to_aa.py

# 3. unique 서열 추출 + IMGT numbering
python3 imgt_numbering.py

# 4. position별 아미노산 비율 계산
python3 aa_proportion.py
```
